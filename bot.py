...
async def op(event):
  ...
  async for x in client.iter_messages("@s0_bm"):
    try:
      await x.forward_to("@s0_bm")
    except:
      pass
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      try:
        ...
        await event.respond("**اذا واجهتك اي مشكله أرسلها هنا : @SELVER7 **" + str(e))
